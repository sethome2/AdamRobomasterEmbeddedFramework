 ...
 * @LastEditTime: 2022-03-01 16:12:13
...
void RMD_setup(RMD_X *motor, RMD_X_ID_CMD ID)
{
	...
	Q24data = TO_Q24(motor->location_p);
	send2CAN(ID, LOCATION_P2RAM, 0, 0, 0, *(uint8_t *)(&Q24data), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1));
	Q24data = TO_Q24(motor->location_p);
	send2CAN(ID, LOCATION_I2RAM, 0, 0, 0, *(uint8_t *)(&Q24data), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1));

	Q24data = TO_Q24(motor->location_p);
	send2CAN(ID, SPEED_P2RAM, 0, 0, 0, *(uint8_t *)(&Q24data), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1));
	Q24data = TO_Q24(motor->location_p);
	send2CAN(ID, SPEED_I2RAM, 0, 0, 0, *(uint8_t *)(&Q24data), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1));

	Q24data = TO_Q24(motor->location_p);
	send2CAN(ID, CURRENT_P2RAM, 0, 0, 0, *(uint8_t *)(&Q24data), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1));
	Q24data = TO_Q24(motor->location_p);
	send2CAN(ID, CURRENT_I2RAM, 0, 0, 0, *(uint8_t *)(&Q24data), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1), *(uint8_t *)(&Q24data + 1));
}

void RMD_write_angle(RMD_X_ID_CMD ID, float angle)
{
	int32_t angleInt = angle * 100;
	send2CAN(ID, WRITE_LOCATION1, 0, 0, 0, *(uint8_t *)(&angleInt), *(uint8_t *)(&angleInt + 1), *(uint8_t *)(&angleInt + 2), *(uint8_t *)(&angleInt + 3));
}
