 ...
 * @date 2022-05-14
 * 
 * QLU Adam Team 齐鲁工业大学（山东省科学院） ADAM机器人团队
 * 
...
layer_node *layer_list_begin = NULL;
...
void referee_init()
{
  ...
  layer_list_begin = malloc(sizeof(layer_node));
  layer_list_begin->next = NULL;
}

int layer_send(graphic_data_struct_t data[], int len);
{
  ext_student_interactive_header_data_t frame;

  referre_encode_frame(COM_EACH_OTHER_ID,)
  return 0;
}

layer_node *referre_layer_add(uint8_t name, uint32_t color, uint32_t start_x, uint32_t start_y, uint32_t end_x, uint32_t end_y)
{
  layer_node *now = layer_list_begin;
  while (now->next != NULL)
  {
    now = now->next;
  }
  now->next = malloc(sizeof(layer_node));
  // do something
  now->graphic.name[0] = name;
  return now->next;
}

layer_node *referre_layer_find(uint8_t name)
{
  layer_node *now = layer_list_begin;
  while (now->next != NULL)
  {
    if (now->graphic.name[0] == name)
      return now;
    now = now->next;
  }
...
int referre_layer_del(uint8_t name)
{
  layer_node *res = referre_layer_find(name);
  if (res == NULL)
    return -1;
...
void layer_able_update(uint8_t name)
{
  layer_node *res = referre_layer_find(name);
  if (res != NULL)
    res->updata = 1;
}
void layer_disable_update(uint8_t name)
{
  layer_node *res = referre_layer_find(name);
  if (res != NULL)
    res->updata = 0;
